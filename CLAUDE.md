...
- 서비스: `app/Services/{도메인}/{액션}/Service.php`
- 리퀘스트: `app/Http/Controllers/{도메인}/{액션}/Request.php`

### 생성 도구
**Blueprint**: `php artisan blueprint:build`
**Plobin**: `php artisan plobin:generate User Create`

### 핵심 규칙
- 1개 파일 = 1개 메서드만
- 클래스명은 `Controller`, `Service`, `Request`로 통일
- Request는 store/update 액션만 생성
...
```
sandbox/container/
├── containers.json                              # 메타데이터 파일
└── {컨테이너명}/
    └── {도메인명}/
        ├── 100-common/
        │   ├── 103-Routes/
        │   │   └── api.php                      # 도메인별 API 라우팅
        │   └── 200-Database/
        │       └── release.sqlite               # SQLite 데이터베이스
        └── {스크린명}/
```

### 라우팅 시스템
**도메인 위임형 라우팅**: 각 도메인이 자신의 API 라우팅을 직접 관리

**SandboxRouteServiceProvider**: 모든 도메인의 103-Routes/api.php 파일을 자동 로드
- 경로: `api/sandbox/{컨테이너명}/{도메인명}/*`
- 각 도메인은 독립적인 라우팅 관리

**도메인별 API 구현 방식**
- SQLite 직접 접근으로 CRUD 구현
- 외부 헬퍼나 서비스 클래스 사용 금지
- 모든 쿼리는 도메인 내부에서 처리

### SandboxRegistry 서비스
**메타데이터 접근 서비스**: containers.json 동적 읽기

**GetContainers**: 전체 컨테이너 목록 반환
**GetDomains**: 특정 컨테이너의 도메인 목록 반환
**GetPages**: 특정 도메인의 페이지(스크린) 목록 반환

**예외 처리**: containers.json 파일 부재나 빈 파일 시 `SandboxMetadataNotFoundException` 발생
